...
public struct Popup<PopupContent: View>: ViewModifier {
    ...
    private var displayedOffset: CGFloat {
        if opaqueBackground {
            switch type {
            case .`default`:
                let topBottomSpace = (screenHeight - sheetContentRect.height)/2 - safeAreaInsets.top
                var padding = 0.0
                if topBottomSpace < keyboardHeightHelper.keyboardHeight {
                    let keyboardOverlap = keyboardHeightHelper.keyboardHeight - topBottomSpace
                    padding = topBottomSpace - keyboardOverlap - safeAreaInsets.top - 20.0
                } else {
                    padding = topBottomSpace
                }
                return padding
            case .toast:
                if position == .bottom {
